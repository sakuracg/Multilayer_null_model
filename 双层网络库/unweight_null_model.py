...
# 适合取增强时最大的极限
def rich_club_createMax(G0, k=1, nswap=1, max_tries=100, connected=1):
    if not nx.is_connected(G0):
        raise nx.NetworkXError("非连通图，必须为连通图")
    if G0.is_directed():
        raise nx.NetworkXError("仅适用于无向图")
    ...
    ci = 3
    while ci > 0:
        for i in range(len_hubs): # 对每一个节点
            hub_node = hubs[i]
            neigh_nodes_i = [e for e in list(G[hub_node]) if G.degree(e) <= k]
            len_neigh_nodes_i = len(neigh_nodes_i)
            if len_neigh_nodes_i == 0:
                continue
            # 遍历该节点的每一个相连节点进行断边重连
            for j in range(len_neigh_nodes_i):
                neigh_node = neigh_nodes_i[j] # 单个节点
                for d in range(i+1, len_hubs):
                    if d == i : # 保证不自环
                        continue
                    hub_node_sec = hubs[d] # 另一个富节点
                    # 找该富节点的相邻节点，如果有进行交换，没有则下一个
                    neigh_nodes_d = [e for e in list(G[hub_node_sec]) if G.degree(e) <= k]
                    len_neigh_nodes_d = len(neigh_nodes_d)
                    if len_neigh_nodes_d == 0:
                        continue
                    # 有则进行选择， 断边重连
                    neigh_node_sec = random.choice(neigh_nodes_d)
                    if (hub_node not in G[hub_node_sec]) and (neigh_node not in G[neigh_node_sec]):  # 保证新生成的连边是原网络中不存在的边
                        G.add_edge(hub_node, hub_node_sec)
                        G.add_edge(neigh_node, neigh_node_sec)

                        G.remove_edge(hub_node, neigh_node)
                        G.remove_edge(neigh_node_sec, hub_node_sec)
                        # hubs_edges.append((u,y)) #更新已存在富节点和富节点连边
                        if connected == 1:  # 判断是否需要保持联通特性，为1的话则需要保持该特性
                            if not nx.is_connected(G):  # 保证网络是全联通的:若网络不是全联通网络，则撤回交换边的操作

                                G.add_edge(hub_node, neigh_node)
                                G.add_edge(neigh_node_sec, hub_node_sec)
                                G.remove_edge(hub_node, hub_node_sec)
                                G.remove_edge(neigh_node, neigh_node_sec)
...
def rich_club_create(G0, k=1, nswap=1, max_tries=100, connected=1):
   ...
   # k 为富节点度值的门限值
    ...
    # 计时
    time_start = int(time.time())
    # and len(hubs_edges) < len_possible_edges
    while swapcount<nswap:
        ...
        u,y = random.sample(hubs,2)         #因为断边重连后，富节点的个数会变              #任选两个富节点
        # 检查u y是否有连边
        if u in G[y]:
            continue
        # v x 从度小k的u，y的相邻节点中
        neigh_nodes_u = [e for e in list(G[u]) if G.degree(e) <= k]
        neigh_nodes_y = [e for e in list(G[y]) if G.degree(e) <= k]
        if len(neigh_nodes_u) == 0 :
            # hubs.remove(u)
            continue
        if len(neigh_nodes_y) == 0 :
            # hubs.remove(y)
            continue
        v = random.choice(neigh_nodes_u)
        x = random.choice(neigh_nodes_y)
        # 检查是否是四个独立节点
        if len(set([u,v,x,y])) != 4:
            continue

        # 判断v，x没有连边# 判断 u，y没有连边
        if (y not in G[u]) and (v not in G[x]):       #保证新生成的连边是原网络中不存在的边
            G.add_edge(u,y)
            G.add_edge(x,v)
...
#    keys,degrees = zip(*G.degree().items()) 
    ...
    while swapcount < nswap:       #有效交换次数小于规定交换次数
        ...
        if len(set([u,v,x,y]))<4:
            continue
        # if G.degree(u) >= G.degree(x) and G.degree(v) >= G.degree(y):
        #     continue
        # if G.degree(u) >= G.degree(y) and G.degree(v) >= G.degree(x):
        #     continue
