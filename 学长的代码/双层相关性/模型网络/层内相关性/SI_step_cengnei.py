...
G_Baw_BApdup,df_Baw_BApdup = creatG(filename ='Baw_BApdup.csv')
G_Baw_wspdup,df_Baw_wspdup = creatG(filename ='Baw_wspdup.csv')
G_Baw_BApddown,df_Baw_BApddown = creatG(filename ='Baw_BApddown.csv')
G_Baw_wspddown,df_Baw_wspddown = creatG(filename ='Baw_wspddown.csv')
...
#    Ilist = [1579360]   #pc用
    ...
    Slist=[item for item in nodes if item not in set(Ilist)]  # 未感染节点列表
    G.add_nodes_from(Slist, state='S')  # 更新节点属性
    sList=[]
    iList=[]
    iiList=[1/len(nodes)]
...
list_BApdup = [si(G_Baw_BApdup,0.02,75,[i]) for i in nodes_Baw[50:150]]
list_wspdup = [si(G_Baw_wspdup,0.02,75,[i]) for i in nodes_Baw[50:150]]
list_BApddown = [si(G_Baw_BApddown,0.02,75,[i]) for i in nodes_Baw[50:150]]
list_wspddown = [si(G_Baw_wspddown,0.02,75,[i]) for i in nodes_Baw[50:150]]
...
pl.plot(iList_Baw_BApdup, 'rd-',markersize = 5,label='BA层内度相关系数增加双层网络')
pl.plot(iList_Baw_wspdup, 'bo-',markersize = 5,label='ws层内度相关系数增加双层网络')
pl.plot(iList_Baw_BApddown, 'ys-',markersize = 5,label='BA层内度相关系数减小双层网络')
pl.plot(iList_Baw_wspddown, 'g*-',markersize = 5,label='ws层内度相关系数减小双层网络')
