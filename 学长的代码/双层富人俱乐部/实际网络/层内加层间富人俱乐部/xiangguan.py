...
df_BArichup = pd.read_csv('call_richup.csv')
df_BArichdown = pd.read_csv('call_richdown.csv')
df_wsrichup = pd.read_csv('sms_richup.csv')
df_wsrichdown = pd.read_csv('sms_richdown.csv')
...
twoup（分开运行）
'''
hubs1 = [e for e in G_BA_pdup.nodes() if G_BA_pdup.degree(e)>11] #全部富节点
hubs2 = [e for e in G_BA_pddown.nodes() if G_BA_pddown.degree(e)>11] #全部富节点
hubs3 = [e for e in G_ws_pdup.nodes() if G_ws_pdup.degree(e)>10] #全部富节点
hubs4 = [e for e in G_ws_pddown.nodes() if G_ws_pddown.degree(e)>10] #全部富节点

G_BA_pdup_interup = nr.replace_rich_in(G_BA_pdup,G_ws_pdup,k=11,k1=10, nswap=2000000, max_tries=1000000000)
G_BA_pdup_interdown = nr.replace_rich_de(G_BA_pdup,G_ws_pdup,k=11,k1=10, nswap=2000000, max_tries=100000000)
G_BA_pdup_interrandom = nr.replace_rich_random(G_BA_pdup,G_ws_pdup,2000000,20000000)
#
rc_twoup = rc(G_BA_pdup,G_ws_pdup,11,10)
rc_BA_richup_interup = rc(G_BA_pdup_interup,G_ws_pdup,11,10)
rc_BA_richup_interdown = rc(G_BA_pdup_interdown,G_ws_pdup,11,10)
rc_BA_richup_interrandom = rc(G_BA_pdup_interrandom,G_ws_pdup,11,10)


df_Baw = mergedf(df_BArichup,df_wsrichup)
df_Baw.to_csv('cas_tworichup.csv',index = False)
##
df_BA_pdup = G2df(G_BA_pdup_interup)
df_BA_pdup.to_csv('call_richup_interup.csv',index = False)
df_BA_pdup['network'] = 0
df_Baw_BApdup = mergedf(df_BA_pdup,df_wsrichup)
df_Baw_BApdup.to_csv('cas_richup_interup.csv',index = False)
##
df_BA_pddown = G2df(G_BA_pdup_interdown)         
df_BA_pddown.to_csv('call_richup_interdown.csv',index = False)           
df_BA_pddown['network'] = 0   
df_Baw_BApddown = mergedf(df_BA_pddown,df_wsrichup)
df_Baw_BApddown.to_csv('cas_richup_interdown.csv',index = False)

df_BA_pdrandom = G2df(G_BA_pdup_interrandom)
df_BA_pdrandom.to_csv('call_richup_interrandom.csv',index = False)
df_BA_pdrandom['network'] = 0
df_Baw_BApdrandom = mergedf(df_BA_pdrandom,df_wsrichup)
df_Baw_BApdrandom.to_csv('cas_richup_interrandom.csv',index = False)


'''
twodown（分开运行）
'''
#G_BA_pddown_interup = nr.replace_rich_in(G_BA_pddown,G_ws_pddown,k=11,k1=10, nswap=2000000, max_tries=100000000)
#G_BA_pddown_interdown = nr.replace_rich_de(G_BA_pddown,G_ws_pddown,k=11,k1=10, nswap=2000000, max_tries=100000000)
#G_BA_pddown_interrandom = nr.replace_rich_random(G_BA_pddown,G_ws_pddown,2000000,2000000)
#
#rc_twodown = rc(G_BA_pddown,G_ws_pddown,11,10)
#rc_BA_richdown_interup = rc(G_BA_pddown_interup,G_ws_pddown,11,10)
#rc_BA_richdown_interdown = rc(G_BA_pddown_interdown,G_ws_pddown,11,10)
#rc_BA_richdown_interrandom = rc(G_BA_pddown_interrandom,G_ws_pddown,11,10)
#
#
#df_Baw = mergedf(df_BArichdown,df_wsrichdown)
#df_Baw.to_csv('cas_tworichdown.csv',index = False)
###
#df_BA_pdup = G2df(G_BA_pddown_interup)
#df_BA_pdup.to_csv('call_richdown_interup.csv',index = False)
#df_BA_pdup['network'] = 0
#df_Baw_BApdup = mergedf(df_BA_pdup,df_wsrichdown)
#df_Baw_BApdup.to_csv('cas_richdown_interup.csv',index = False)
###
#df_BA_pddown = G2df(G_BA_pddown_interdown)             
#df_BA_pddown.to_csv('call_richdown_interdown.csv',index = False)   
#df_BA_pddown['network'] = 0
#df_Baw_BApddown = mergedf(df_BA_pddown,df_wsrichdown)
#df_Baw_BApddown.to_csv('cas_richdown_interdown.csv',index = False)
#
#df_BA_pdrandom = G2df(G_BA_pddown_interrandom)
#df_BA_pdrandom.to_csv('call_richdown_interrandom.csv',index = False)
#df_BA_pdrandom['network'] = 0
#df_Baw_BApdrandom = mergedf(df_BA_pdrandom,df_wsrichdown)
#df_Baw_BApdrandom.to_csv('cas_richdown_interrandom.csv',index = False)
