...
richclub = rc(G_call,G_sms,11,10)
#
G_call_richup = nr.replace_rich_in(G_call,G_sms, k = 11,k1= 10, nswap=2000000, max_tries=1000000000)
rc_callup = rc(G_call_richup,G_sms,11,10)

G_call_richdown = nr.replace_rich_de(G_call,G_sms, k = 11,k1= 10, nswap=2000000, max_tries=100000000)
rc_calldown = rc(G_call_richdown,G_sms,11,10)

G_call_richrandom = nr.replace_rich_random(G_call,G_sms,nswap=2000000, max_tries=100000000)
rc_callrandom = rc(G_call_richrandom,G_sms,11,10)
'''
合并双层网络并加权处理
'''
def mergedf(df1,df2):
...
df_cas_callrichup.to_csv('cas_richup.csv',index = False)
#
...
df_cas_callrichdown.to_csv('cas_richdown.csv',index = False)

#
df_call_richrandom = G2df(G_call_richrandom)
df_call_richrandom.to_csv('call_richrandom.csv',index = False)
df_call_richrandom['network'] = 0
df_cas_callrichrandom = mergedf(df_call_richrandom,df_sms)
df_cas_callrichrandom.to_csv('cas_richrandom.csv',index = False)
#
