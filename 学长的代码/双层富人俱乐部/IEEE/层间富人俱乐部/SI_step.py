...
G_Baw_richup,df_Baw_richup = creatG(filename = 'cas_richup.csv')
G_Baw_richdown,df_Baw_richdown = creatG(filename = 'cas_richdown.csv')
G_Baw_richrandom,df_Baw_richrandom = creatG(filename = 'cas_richrandom.csv')
...
list_origin = [si(G_Baw,0.1,120,[i]) for i in nodes_Baw[150:250]]
list_richup = [si(G_Baw_richup,0.1,120,[i]) for i in nodes_Baw[150:250]]
list_richdown = [si(G_Baw_richdown,0.1,120,[i]) for i in nodes_Baw[150:250]]
list_richrandom = [si(G_Baw_richrandom,0.1,120,[i]) for i in nodes_Baw[150:250]]
iList_Baw=list(np.array(list_origin).mean(axis=0))
iList_Baw_richup=list(np.array(list_richup).mean(axis=0))
iList_Baw_richdown=list(np.array(list_richdown).mean(axis=0))
iList_Baw_richrandom=list(np.array(list_richrandom).mean(axis=0))
pl.plot(iList_Baw, 'k-',label='原始双层网络')
pl.plot(iList_Baw_richup, 'rd-',markersize = 5,label='层间富人俱乐部系数增加双层网络')
pl.plot(iList_Baw_richdown, 'bo-',markersize = 5,label='层间富人俱乐部系数减小双层网络')
pl.plot(iList_Baw_richrandom, 'ys-',markersize = 5,label='层间富人俱乐部系数随机双层网络')
