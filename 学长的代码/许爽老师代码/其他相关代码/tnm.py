...
def edges_swap_1k(G0, nswap=2000000, max_tries=10000000):  #时变网络的连边置乱算法
    """
    ...
    while swapcount < nswap:
        ...
        if y not in G[u] and v not in G[x]: 
            for i in list(G[u][v].keys()):
                G.add_edge(u,y,key=i,timestamp=G[u][v][i]['timestamp'])
                G.remove_edge(u,v,key=i)
            for j in list(G[x][y].keys()):
...
def time_swap(G0, nswap=2000000, max_tries=100000000):  #时变网络的时间置乱算法
    """
    ...
    while swapcount < nswap:
        ...
        if len(set([u,v,x,y])) < 3: #保证从两条边上选时间戳           
            continue
        key1 = random.choice(list(G[u][v].keys()))
        key2 = random.choice(list(G[x][y].keys()))
...
def time_random(G0, nswap=2000000, max_tries=1000000000,mins=86403,maxs=2675998):  #时变网络的时间随机化算法
    """
    ...
    while swapcount < nswap:
        ...
        n+=1
        #改进后
        (ui,)=nx.utils.discrete_sequence(1,cdistribution=cdf)
...
def timeweight_swap(G0, nswap=2000000, max_tries=1000000000):  #时变网络的时权置乱算法
    """
    任选两条边，将其上的时间序列互换
    """
    print(5)
...
def sametimeweight_swap(G0, nswap=2000000, max_tries=1000000000):  #时变网络的等时权置乱算法
    """
...
def touch_swap(G0, nswap=2000000, max_tries=1000000000):  #时变网络的接触置乱算法
    """
